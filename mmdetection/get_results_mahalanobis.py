...
post_process_Mahalanobis_norm(score_id, score_ood, logits=False)
...
# Load test (OOD) features
test_path = os.path.join(BASE_RESULTS_FOLDER, model_type, 'mahalanobis', args.dataset, 'testOOD')
feature_ood = np.load(os.path.join(test_path, f'{args.saveNm}_feature_ood_logits.npy'))
...
post_process_Mahalanobis_norm(score_id, score_ood, logits=True)
