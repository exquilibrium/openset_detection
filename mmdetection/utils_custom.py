...
def fit_gmms(logits, labels, ious, confs, scoreThresh, iouThresh, num_classes, class_id, components = 1, covariance_type = 'full'):
    gmms = [None for i in range(num_classes)]
    for i in range(num_classes):
        ls = logits[labels == class_id[i]]
        iou = ious[labels == class_id[i]]
        conf = confs[labels == class_id[i]]
